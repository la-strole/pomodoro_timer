...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function runningTimerDisplays () {
    ...
    if (!t.timer_control.breaking_flag) {
      // Change breaking image if it is not exist
      if (displayElements.tree_tag.src !== 'static/break.gif') {
        displayElements.tree_tag.classList.add('mx-auto', 'my-auto', 'd-block', 'w-75')
        displayElements.tree_tag.src = 'static/break.gif'
      }
    } else { // if it is pomo time
      displayElements.tree_tag.classList.remove('mx-auto', 'my-auto', 'd-block', 'w-75')
      const timeStep = Math.floor((t.timer_init.m * 60 + t.timer_init.s) / 25)
      const time = t.timer_state.minutes * 60 + t.timer_state.seconds
      if (time % timeStep === 0 && time > 0) {
        // change tree image
        if (time / timeStep >= 25) {
          displayElements.tree_tag.src = 'static/tree/2.jpg'
        } else {
          displayElements.tree_tag.src = `static/tree/${26 - (time / timeStep)}.jpg`
        }
      }
    }
